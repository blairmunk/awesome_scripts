...
# Version: 2.4 (Fixed encoding, added autostart)
...
# Functions for colored output (ASCII only)
print_color() {
    printf "${1}%s${NC}\n" "$2"
}
...
check_root() {
    if [[ $EUID -eq 0 ]]; then
        ...
        print_info "  ./install-notesnook.sh"
        exit 1
    fi
}
...
setup_docker_permissions() {
    ...
    if ! docker info >/dev/null 2>&1; then
        ...
        # Temporary solution - change socket permissions
...
download_web_app() {
    if [[ -n "$WEB_DOMAIN" ]]; then
        ...
        # Create HTML without Unicode characters
...
<head>
    ...
    <title>INSTANCE_NAME_PLACEHOLDER</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="server-url">
            https://notes.DOMAIN_PLACEHOLDER
        </div>
        ...
        <div class="instructions">
            ...
            <div class="step">Enter URL: <strong>https://notes.DOMAIN_PLACEHOLDER</strong></div>
            <div class="step">Create a new account or sign in to existing one</div>
        </div>

        <div class="info-box">
            <h3>Useful Links:</h3>
            <p><strong>API Server:</strong> <a href="https://notes.DOMAIN_PLACEHOLDER/health">https://notes.DOMAIN_PLACEHOLDER</a></p>
            <p><strong>Public Notes:</strong> <a href="https://mono.DOMAIN_PLACEHOLDER">https://mono.DOMAIN_PLACEHOLDER</a></p>
            <p><strong>Files (MinIO):</strong> <a href="https://files.DOMAIN_PLACEHOLDER">https://files.DOMAIN_PLACEHOLDER</a></p>
        </div>
    </div>
    ...
    <script>
        ...
        if (window.location.search.includes('setup=true')) {
            localStorage.setItem('serverURL', 'https://notes.DOMAIN_PLACEHOLDER');
            alert('Sync server configured: https://notes.DOMAIN_PLACEHOLDER');
        }
    </script>
</body>
</html>
HTML_EOF
        ...
        sed -i "s/INSTANCE_NAME_PLACEHOLDER/$INSTANCE_NAME/g" index.html
        sed -i "s/DOMAIN_PLACEHOLDER/$DOMAIN/g" index.html
        
        print_success "Web application created in /var/www/notesnook-web/"
    fi
}

# Create configuration files with autostart
create_config_files() {
...
EOL
    ...
    # Create docker-compose.yml with restart policies
...
# Configure autostart
configure_autostart() {
    print_step "Configuring container autostart"
    
    # Enable Docker service autostart
    sudo systemctl enable docker
    print_success "Docker service autostart enabled"
    
    # Create systemd service for Notesnook
    sudo tee /etc/systemd/system/notesnook.service > /dev/null << EOF
[Unit]
Description=Notesnook Self-Hosted Server
...
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/srv/Files/Notesnook/setup
...
EOF
    ...
    sudo systemctl enable notesnook.service
    print_success "Notesnook systemd service created and enabled"
    
    print_info "Containers will now start automatically after server reboot"
}

# Сокращенные версии остальных функций для экономии места
# Pull Docker images
pull_images() {
...
# Create management utilities (ASCII only)
...
echo -e "${YELLOW}[!] Re-login to apply docker group permissions${NC}"
...
if systemctl is-enabled docker >/dev/null 2>&1; then
    echo -e "${GREEN}[+] Docker service autostart enabled${NC}"
else
    echo -e "${RED}[-] Docker service autostart disabled${NC}"
    echo -e "${YELLOW}   Fix: sudo systemctl enable docker${NC}"
fi

if systemctl is-enabled notesnook >/dev/null 2>&1; then
    echo -e "${GREEN}[+] Notesnook service autostart enabled${NC}"
else
    echo -e "${YELLOW}[!] Notesnook systemd service not configured${NC}"
fi
...
if docker compose ps >/dev/null 2>&1; then
    ...
    echo -e "${YELLOW}[!] Issues accessing Docker${NC}"
...
if [ -f .env ]; then
    ...
    for sub in "${subdomains[@]}"; do
        ...
        if curl -sf "https://$full_domain" -m 5 >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}[!] HTTPS issues for ${full_domain}${NC}"
        fi
    done
...
echo -e "${BLUE}[*] Downloading Docker image updates...${NC}"
...
echo -e "${YELLOW}[i] Size: $(du -h "$BACKUP_DIR.tar.gz" | cut -f1)${NC}"
EOL
...
show_final_info() {
    ...
    print_color $CYAN "[i] SYSTEM INFORMATION:"
    echo "   Main domain: $DOMAIN"
    echo "   Instance name: $INSTANCE_NAME"
    echo "   Registration: $([ "$DISABLE_SIGNUPS" = "true" ] && echo "Disabled" || echo "Enabled")"
    echo
    
    print_color $CYAN "[i] AVAILABLE SERVICES:"
    ...
    print_color $CYAN "[i] USEFUL COMMANDS:"
    ...
    print_color $CYAN "[i] CLIENT SETUP:"
    ...
    if [[ -n "$WEB_DOMAIN" ]]; then
        print_color $CYAN "[i] WEB APPLICATION:"
    ...
    echo "   * Containers will restart automatically after reboot"
    echo
...
# Main function
main() {
    print_header "NOTESNOOK SELF-HOSTED SERVER INSTALLER v2.4"
    print_info "Automated installation of Notesnook sync server with autostart"
